 ...
 * @author smahaffey
 */
public class ExCorFullHeatMapData {
    ArrayList<HeatMapRow> hmr=new ArrayList<HeatMapRow>();
    Hashtable probeTable=new Hashtable();
    
    
    public ExCorFullHeatMapData(){
    ...
    public void readFromFile(String surl){
        ...
        try {
            ...
            Logger.getLogger(ExCorFullHeatMapData.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            try {
                br.close();
            } catch (IOException ex) {
    ...
    public ExCorHeatMapData generateMap(ArrayList<ProbeSet> fullProbes){
        ...
        ExCorHeatMapData ret=new ExCorHeatMapData(ExCorHeatMapData.TWO_COLOR);
        ret.setMinMax(-1,1,0);
        ret.setData(tmpValues, probeList, missingList);
        return ret;
    }
